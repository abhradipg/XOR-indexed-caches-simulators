...
isl_ctx *allocateContextWithIncludePaths(std::vector<std::string> IncludePaths) {
  ...
  options = model_options_new_with_defaults();
  ArgumentCount = model_options_parse(options, ArgumentCount, &Arguments[0], ISL_ARG_ALL);
  return isl_ctx_alloc_with_options(&model_options_args, options);
}

void CacheModel::compileProgram(std::string SourceFile) { Program_.extractScop(SourceFile); }

void CacheModel::initModel(std::vector<NamedLong> Parameters) {
...
std::vector<NamedMisses> CacheModel::countCacheMisses() {
...
std::vector<NamedVector> CacheModel::countCacheMisses(std::vector<long> CacheSizes) {
...
void CacheModel::computeGlobalMaps() {
  Timer::startTimer("ComputeBetweenMap");
...
void CacheModel::extractAccesses() {
...
void CacheModel::addConflicts(isl::union_map BetweenMap) {
