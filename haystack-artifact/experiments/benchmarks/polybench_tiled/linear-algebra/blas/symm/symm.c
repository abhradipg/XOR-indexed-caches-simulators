...
static
__attribute__((noinline)) void kernel_symm(int m, int n,
		 DATA_TYPE alpha,
		 DATA_TYPE beta,
		 DATA_TYPE POLYBENCH_2D(C,M,N,m,n),
		 DATA_TYPE POLYBENCH_2D(A,M,M,m,m),
         ...
         for (int c2 = 16 * c0; c2 <= min(999, 16 * c0 + 15); c2 += 1)
           for (int c3 = 16 * c1; c3 <= 16 * c1 + 15; c3 += 1) {
             ...
             for (int c4 = 0; c4 < c2; c4 += 1)
               temp2 += (B[c4][c3] * A[c2][c4]);
             C[c2][c3] = (((beta * C[c2][c3]) + ((alpha * B[c2][c3]) * A[c2][c2])) + (alpha * temp2));
           }
     for (int c0 = 0; c0 <= 74; c0 += 1)
       for (int c1 = 0; c1 <= 62; c1 += 1)
         for (int c2 = c1; c2 <= 62; c2 += 1)
           for (int c3 = 16 * c0; c3 <= 16 * c0 + 15; c3 += 1)
             for (int c4 = 16 * c1; c4 <= min(min(998, 16 * c1 + 15), 16 * c2 + 14); c4 += 1)
               for (int c5 = max(16 * c2, c4 + 1); c5 <= min(999, 16 * c2 + 15); c5 += 1)
                 C[c4][c3] += ((alpha * B[c5][c3]) * A[c5][c4]);
   }
#pragma endscop
