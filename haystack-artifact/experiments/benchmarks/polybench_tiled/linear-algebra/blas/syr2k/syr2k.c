...
static
__attribute__((noinline)) void kernel_syr2k(int n, int m,
		  DATA_TYPE alpha,
		  DATA_TYPE beta,
		  DATA_TYPE POLYBENCH_2D(C,N,N,n,n),
		  DATA_TYPE POLYBENCH_2D(A,N,M,n,m),
    ...
    for (int c0 = 0; c0 <= 74; c0 += 1)
      for (int c1 = 0; c1 <= c0; c1 += 1)
        for (int c2 = 16 * c0; c2 <= 16 * c0 + 15; c2 += 1)
          for (int c3 = 16 * c1; c3 <= min(16 * c1 + 15, c2); c3 += 1)
            C[c2][c3] *= beta;
    for (int c0 = 0; c0 <= 74; c0 += 1)
      for (int c1 = 0; c1 <= c0; c1 += 1)
        for (int c2 = 0; c2 <= 62; c2 += 1)
          for (int c3 = 16 * c0; c3 <= 16 * c0 + 15; c3 += 1)
            for (int c4 = 16 * c1; c4 <= min(16 * c1 + 15, c3); c4 += 1)
              for (int c5 = 16 * c2; c5 <= min(999, 16 * c2 + 15); c5 += 1)
                C[c3][c4] += (((A[c4][c5] * alpha) * B[c3][c5]) + ((B[c4][c5] * alpha) * A[c3][c5]));
  }
#pragma endscop
