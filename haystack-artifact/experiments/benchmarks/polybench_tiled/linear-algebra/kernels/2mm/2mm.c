...
static
__attribute__((noinline)) void kernel_2mm(int ni, int nj, int nk, int nl,
		DATA_TYPE alpha,
		DATA_TYPE beta,
...
#pragma scop
  ...
  #define notused_min(x,y)    ({ __typeof__(x) _x = (x); __typeof__(y) _y = (y); _x < _y ? _x : _y; })
  {
    ...
    for (int c0 = 0; c0 <= 49; c0 += 1)
      for (int c1 = 0; c1 <= 56; c1 += 1)
        for (int c2 = 0; c2 <= 68; c2 += 1)
          for (int c3 = 16 * c0; c3 <= 16 * c0 + 15; c3 += 1)
            for (int c4 = 16 * c1; c4 <= min(899, 16 * c1 + 15); c4 += 1)
              for (int c5 = 16 * c2; c5 <= min(1099, 16 * c2 + 15); c5 += 1)
                tmp[c3][c4] += ((alpha * A[c3][c5]) * B[c5][c4]);
    for (int c0 = 0; c0 <= 49; c0 += 1)
