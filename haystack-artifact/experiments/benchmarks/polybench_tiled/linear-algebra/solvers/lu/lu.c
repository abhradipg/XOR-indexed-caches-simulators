...
static
__attribute__((noinline)) void kernel_lu(int n,
	       DATA_TYPE POLYBENCH_2D(A,N,N,n,n))
{
  int i, j, k;
...
#pragma scop
  ...
  for (int c0 = 0; c0 <= 124; c0 += 1)
    for (int c1 = 0; c1 <= c0; c1 += 1)
      for (int c2 = max(1, 16 * c0); c2 <= 16 * c0 + 15; c2 += 1)
        for (int c3 = max(1, 16 * c1); c3 <= min(16 * c1 + 15, c2); c3 += 1) {
          A[c2][c3 - 1] /= A[c3 - 1][c3 - 1];
          for (int c4 = c2; c4 <= 1999; c4 += 1)
            A[c2][c4] -= (A[c2][c3 - 1] * A[c3 - 1][c4]);
          if (c2 >= c3 + 1)
            for (int c4 = 0; c4 < c3; c4 += 1)
              A[c2][c3] -= (A[c2][c4] * A[c4][c3]);
        }
#pragma endscop
}
