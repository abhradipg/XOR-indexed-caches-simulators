...
#pragma scop
  ...
  for (int c0 = 1; c0 <= 1999; c0 += 1)
    for (int c1 = 0; c1 <= c0 + 1998; c1 += 32)
      for (int c2 = ppcg_max(0, ((c0 - c1 + 2014) % 32) - c0 + c1 - 30); c2 <= ppcg_min(1999, c1 + 31); c2 += 32)
        for (int c3 = 0; c3 <= ppcg_min(ppcg_min(31, c0 - c1 + 1998), c0 - c1 + c2 + 30); c3 += 1)
          for (int c4 = ppcg_max(0, -c0 + c1 - c2 + c3 + 1); c4 <= ppcg_min(ppcg_min(31, -c2 + 1999), c1 - c2 + c3); c4 += 1)
            B[c0][c2 + c4] += ((alpha * A[c0][c1 - c2 + c3 - c4]) * B[c2 + c4][c1 - c2 + c3 - c4]);
#pragma endscop
