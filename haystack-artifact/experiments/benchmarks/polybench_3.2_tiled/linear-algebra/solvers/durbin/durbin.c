...
#pragma scop
  ...
  
  {
    ...
    beta[0] = 1;
    y[0][0] = r[0];
    for (int c0 = 1; c0 <= 7999; c0 += 1) {
      beta[c0] = (beta[c0 - 1] - ((alpha[c0 - 1] * alpha[c0 - 1]) * beta[c0 - 1]));
      sum[0][c0] = r[c0];
      for (int c1 = 0; c1 < c0; c1 += 1)
        sum[c1 + 1][c0] = (sum[c1][c0] + (r[c0 - c1 - 1] * y[c1][c0 - 1]));
      alpha[c0] = ((-sum[c0][c0]) * beta[c0]);
      for (int c1 = 7998; c1 <= c0 + 7997; c1 += 1)
        y[c1 - 7998][c0] = (y[c1 - 7998][c0 - 1] + (alpha[c0] * y[c0 - c1 + 7997][c0 - 1]));
      y[c0][c0] = alpha[c0];
    }
    for (int c0 = 0; c0 <= 7999; c0 += 1)
      out[c0] = y[c0][7999];
  }
#pragma endscop
